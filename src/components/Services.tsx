  ...
  return (
    <section className="relative bg-background px-[4vw] pt-24 pb-[40vh]">
      <FadeUp className="mb-16">
        <span className="text-eyebrow text-muted uppercase tracking-widest">What we do</span>
      ...
      <div className="relative">
        {services.map((service, i) => (
          <article
            ...
            className="sticky w-full min-h-[50vh] bg-darkCard text-foreground rounded-card p-8 md:p-12 mb-4 flex flex-col justify-between"
          >
            <div className="flex justify-between items-start">
              <h3 className="text-card font-medium">{service.title}</h3>
              {/* Continuously rotating asterisk */}
              <span
                className="text-4xl text-muted animate-spin-slow"
              >
                ✳
              </span>
            </div>
            ...
            <div className="mt-auto">
              <p className="text-body text-muted-foreground max-w-lg mb-6">{service.description}</p>
              <ul className="flex flex-wrap gap-2">
                {service.tags.map((tag) => (
                  <li
                    key={tag}
                    className="text-eyebrow border border-muted-foreground rounded-full px-4 py-1.5 text-muted-foreground"
                  >
                    {tag}
                  </li>
                ))}
